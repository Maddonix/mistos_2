...
@router.post("/api/deepflash/estimate_ground_truth", status_code=201)
async def estimate_ground_truth(estimate_ground_truth_request: EstimateGroundTruthRequest):
    ...
    for image_id, label_id_list in images_label_dict.items():
        ...
        int_image.estimate_ground_truth_layer(label_id_list, suffix="")
...
async def upload_deepflash_model(read_from_path_request: ReadFromPathRequest):
    ...
    c_int_classifier_deepflash = classes.IntClassifier(
        uid=-1,
        name=name,
    ...
    return {"Result": "OK", "uid": c_int_classifier_deepflash.uid}


@router.post("/api/deepflash/predict_images", status_code=201)
async def predict_images(predict_images_request: PredictImagesRequest):
    ...
    utils_deepflash.predict_image_list(
        classifier_id, image_ids, use_tta, channel, separate_z_slices=False)


@router.post("/api/deepflash/predict_images_3d", status_code=201)
