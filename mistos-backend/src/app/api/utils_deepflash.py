...
def predict_image_list(classifier_id, image_id_list, use_tta, channel = 0, transform_to_multilabel = True, separate_z_slices = False):
    ...
    if separate_z_slices == False:
        ...
        for image_id in image_id_list:
            ...
            for n_layer in range(image.data.shape[0]):
                ...
                print(layer.shape)
                path = utils_paths.make_tmp_file_path(f"{image.uid}_{n_layer}.zarr")
                print(path)
    ...
    el = EnsembleLearner(files=image_path_list)#, dl_kwargs={'num_workers':0}) # num_workers set to 0 due to cuda error on windows workiing with shared storage 
    ...
    if separate_z_slices == False:
        ...
        for path in el.df_ens["res_path"]:
            image_id, n_layer, segmentation = get_segmentation_from_tmp_path(path)
        ...
        for image_id, segmentation_list in layer_dict.items():
            ...
            result_layer_data = np.zeros((len(segmentation_list), y_dim, x_dim), dtype = bool)
            ...
            if transform_to_multilabel:
                result_layer_data = utils_transformations.binary_mask_to_multilabel(result_layer_data)[0]
